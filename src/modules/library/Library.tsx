...
import { Plus, Search, BookOpen, Users, Download, Upload, Trash2, Edit } from 'lucide-react'
...
export function Library() {
  ...
  const filteredItems = items.filter(item => {
    ...
    if (!selectedProject) return matchesSearch
    
    const project = projects.find(p => p.id === selectedProject)
    return matchesSearch && project?.items.includes(item.id)
  })

  const handleAddItem = (itemData: Omit<VocabItem, 'id' | 'createdAt' | 'updatedAt'>) => {
    addItem(itemData)
    setShowItemForm(false)
  }

  const handleUpdateItem = (itemData: Omit<VocabItem, 'id' | 'createdAt' | 'updatedAt'>) => {
    if (editingItem) {
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {items.length > 0 
                ? Math.round((items.filter(item => item.passed2 > 0).length / items.length) * 100)
                : 0}%
            </div>
            <p className="text-xs text-muted-foreground">
              Items mastered
