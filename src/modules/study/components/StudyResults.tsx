...
import { StudySession } from '../../../core/models'
...
export function StudyResults({ session, onClose, onStartNew }: StudyResultsProps) {
  const accuracy = session.totalQuestions > 0 
    ? Math.round((session.correctAnswers / session.totalQuestions) * 100) 
    : 0
    
  const timeSpentMinutes = Math.floor((Date.now() - session.startedAt) / 60000)
