...
import {
  Download, Upload, Trash2, Save, RefreshCw, Settings as SettingsIcon,
  Globe, Bell, Shield, Clock, Database, CheckCircle, AlertTriangle
} from 'lucide-react'
...
export function Settings() {
  ...
  const [autoBackupInterval, setAutoBackupInterval] = useState(state.settings.autoBackupInterval)
  ...
  const createBackup = (isAuto = false) => {
    try {
      const success = StorageService.createBackup(state)
      if (success) {
  ...
  const restoreBackup = (backupId: string) => {
    ...
    try {
      const restoredState = StorageService.loadBackup()
      if (restoredState) {
  ...
  const handleExport = (includePassword = false) => {
    const data = StorageService.exportData(state)
    let exportData = data
    ...
    a.download = includePassword
      ? `vocab-quiz-backup-encrypted-${new Date().toISOString().split('T')[0]}.json`
      : `vocab-quiz-backup-${new Date().toISOString().split('T')[0]}.json`
  ...
  const handleImport = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const importedState = StorageService.importData(importData)
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => dispatch({ type: 'UPDATE_SETTINGS', payload: { speedMode: !state.settings.speedMode } })}
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                state.settings.speedMode ? 'bg-blue-600' : 'bg-gray-200'
              }`}
            >
              <span
                className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                  state.settings.speedMode ? 'translate-x-6' : 'translate-x-1'
                }`}
              />
            </button>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => dispatch({ type: 'UPDATE_SETTINGS', payload: { compactUI: !state.settings.compactUI } })}
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                state.settings.compactUI ? 'bg-blue-600' : 'bg-gray-200'
              }`}
            >
              <span
                className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                  state.settings.compactUI ? 'translate-x-6' : 'translate-x-1'
                }`}
              />
            </button>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">
                Automatically backup data every: {
                  autoBackupInterval === 0 ? 'Never' :
                  autoBackupInterval < 60 ? `${autoBackupInterval} minutes` :
                  `${autoBackupInterval / 60} hours`
                }
              </p>
            </div>
            <select
          ...
          <div>
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto">
              {backupHistory.length === 0 ? (
                <p className="text-sm text-gray-500">No backups yet. Create your first backup above.</p>
              ) : (
                backupHistory.map((backup) => (
                  <div
                    key={backup.id}
                    className={`flex items-center justify-between p-3 rounded-lg border ${
                      backup.success ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'
                    }`}
                  >
                    <div className="flex items-center gap-2">
                      {backup.success ? (
