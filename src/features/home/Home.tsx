...
import {
  Plus, BookOpen, Play, BarChart3, TrendingUp, Calendar,
  Clock, Target, Award, ChevronRight, Settings
} from 'lucide-react'
...
export function Home() {
  ...
  const todayStats = React.useMemo(() => {
    ...
    const todayItems = state.vocabItems.filter(item =>
      item.lastDayPractice === today
    )

    const inQueueCount = state.vocabItems.reduce((sum, item) => sum + item.inQueue, 0)
    const quitQueueCount = state.vocabItems.reduce((sum, item) => sum + item.quitQueue, 0)
    const streakDays = calculateStreakDays(state.vocabItems)
  ...
  function calculateStreakDays(items: any[]) {
    ...
    while (streak < 30) { // Check up to 30 days back
      ...
      const hasPractice = items.some(item => item.lastDayPractice === dateStr)
  ...
  const needsAttention = React.useMemo(() => {
    return state.vocabItems
      .filter(item => item.successRate < 0.6 || item.inQueue > 0)
      .sort((a, b) => (b.inQueue - a.inQueue) || (a.successRate - b.successRate))
      .slice(0, 5)
  }, [state.vocabItems])

  const handlePracticeSample = () => {
    dispatch({ type: 'TOGGLE_SAMPLE_MODE' })
  }

  const handleStudyMyWords = () => {
  ...
  const handleViewDetails = (type: 'today' | 'performance' | 'attention') => {
    switch (type) {
      case 'today':
        window.location.hash = '/study'
        break
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        <div
          ...
          onClick={() => handleViewDetails('today')}
          title="Click to view today's study sessions"
        >
        ...
        <div
          className="bg-white p-4 rounded-lg shadow cursor-pointer hover:shadow-md transition-shadow"
          onClick={() => handleViewDetails('today')}
          title="Click to view quit sessions"
        >
          <div className="flex items-center justify-between">
            <div>
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <button
            onClick={handleStudyMyWords}
            disabled={todayStats.totalItems === 0}
            className="flex items-center justify-center gap-2 p-4 bg-green-50 hover:bg-green-100 disabled:bg-gray-50 rounded-lg transition-colors disabled:cursor-not-allowed"
          >
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 p-4 bg-purple-50 hover:bg-purple-100 rounded-lg transition-colors"
          >
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`w-3 h-3 rounded-full ${
                    project.mode === 'Random' ? 'bg-orange-500' : 'bg-indigo-500'
                  }`}></div>
                  <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <div className="space-y-3">
            {topPerformers.length === 0 ? (
              <p className="text-gray-500 text-sm">No top performers yet. Start studying to see your progress!</p>
            ) : (
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          <div className="flex justify-between items-center mb-4">
            ...
            <button
              onClick={() => handleViewDetails('attention')}
              className="text-sm text-blue-600 hover:text-blue-700 flex items-center gap-1"
            >
              View All <ChevronRight size={16} />
            </button>
          ...
          <div className="space-y-3">
            {needsAttention.length === 0 ? (
              <p className="text-gray-500 text-sm">Great! All items are performing well.</p>
            ) : (
              needsAttention.map((item) => (
                <div key={item.id} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`w-3 h-3 rounded-full ${
                      item.successRate < 0.4 ? 'bg-red-500' : 'bg-yellow-500'
                    }`}></div>
                    <div>
                      <span className="font-medium text-sm">{item.keyword}</span>
                      <div className="text-xs text-gray-500">{item.tag || 'General'}</div>
                  ...
                  <div className="text-right">
                    <div className={`text-sm font-medium ${
                      item.successRate < 0.4 ? 'text-red-600' : 'text-yellow-600'
                    }`}>
                      {Math.round(item.successRate * 100)}%
                    </div>
                    <div className="text-xs text-gray-500">
                      {item.inQueue > 0 && `${item.inQueue} in queue`}
                    </div>
                  </div>
                </div>
              ))
      ...
      {state.sampleMode && (
        <div className="bg-blue-50 border border-blue-200 p-4 rounded-lg">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => dispatch({ type: 'TOGGLE_SAMPLE_MODE' })}
