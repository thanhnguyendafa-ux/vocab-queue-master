...
export function Analytics() {
    var _a = useQuizStore(), sessionHistory = _a.sessionHistory, items = _a.items;
    // Calculate metrics
    var masteryData = items.map(function (item) { return ({
        id: item.id,
        term: item.term,
        mastery: item.mastery * 100
    }); });
    var sessionData = sessionHistory.map(function (session) { return ({
    ...
    return (<div className="p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white p-4 rounded-lg shadow">
          ...
          <BarChart data={masteryData} xField="term" yField="mastery"/>
        </div>
        
        <div className="bg-white p-4 rounded-lg shadow">
          <h2 className="text-lg font-semibold mb-4">Session Accuracy</h2>
          <LineChart data={sessionData} xField="date" yField="accuracy"/>
        </div>
      </div>
    </div>);
}
